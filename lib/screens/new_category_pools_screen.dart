...
class NewCategoryPoolsScreen extends StatefulWidget {
  final Tournament tournament;

  const NewCategoryPoolsScreen({
    super.key,
    required this.tournament,
  });

  @override
  State<NewCategoryPoolsScreen> createState() => _NewCategoryPoolsScreenState();
}

class _NewCategoryPoolsScreenState extends State<NewCategoryPoolsScreen> {
  ...
  Map<String, List<Team>> _teamsByCategory = {};
  ...
  static const String _keySelectedCategory = 'division_pools_selected_division';
  ...
  Map<String, List<Map<String, dynamic>>> _categoryNodes = {};
  Map<String, Map<String, String>> _categoryTeamPositions = {};
  ...
  Future<void> _loadSavedState() async {
    try {
      ...
      final savedCategory = prefs.getString(_keySelectedCategory);
      if (savedCategory != null) {
        _selectedCategory = savedCategory;
      }
      
      // Load category-specific nodes
      final categoryNodesJson = prefs.getString(_keyCreatedNodes);
      if (categoryNodesJson != null) {
        final categoryNodesData = json.decode(categoryNodesJson) as Map<String, dynamic>;
        _categoryNodes = categoryNodesData.map((category, nodes) {
          ...
          return MapEntry(category, nodesList);
        });
      }
      
      // Load category-specific team positions
      final categoryTeamPositionsJson = prefs.getString(_keyTeamPositions);
      if (categoryTeamPositionsJson != null) {
        _categoryTeamPositions = Map<String, Map<String, String>>.from(
          json.decode(categoryTeamPositionsJson).map((key, value) => MapEntry(key, Map<String, String>.from(value)))
        );
      }
      
      // Set current category state
      if (_selectedCategory != null) {
        _createdNodes = _categoryNodes[_selectedCategory] ?? [];
        _teamPositions = _categoryTeamPositions[_selectedCategory] ?? {};
      }
  ...
  Future<void> _loadTeams() async {
    try {
      ...
      // Group teams by category using the tournament's categoryTeams map
      final Map<String, List<Team>> groupedTeams = {};
      
      // Group all tournament teams under the single 'Standard' category
      {
        final teams = allTeams.where((team) => widget.tournament.teamIds.contains(team.id)).toList();
        groupedTeams['Standard'] = teams;
      }
      ...
      setState(() {
        _teamsByCategory = groupedTeams;
        _isLoading = false;
        
        // Always auto-select 'Standard' if nothing is selected
        if (_selectedCategory == null) {
          _selectedCategory = 'Standard';
        }
        
        // Initialize team positions if not loaded from saved state
        if (_selectedCategory != null && _teamPositions.isEmpty) {
          final categoryTeams = _teamsByCategory[_selectedCategory] ?? [];
          for (final team in categoryTeams) {
            _teamPositions[team.id] = 'sidebar';
          }
          _categoryTeamPositions[_selectedCategory!] = Map.from(_teamPositions);
          }
  ...
  Future<void> _saveState() async {
    try {
      ...
      if (_selectedCategory != null) {
        ...
        // Update category-specific state with current state
        _categoryNodes[_selectedCategory!] = List<Map<String, dynamic>>.from(_createdNodes);
        _categoryTeamPositions[_selectedCategory!] = Map.from(_teamPositions);
      }
      
      // Save category-specific nodes - Convert Team objects to maps
      final categoryNodesData = _categoryNodes.map((category, nodes) {
        ...
        return MapEntry(category, nodesData);
      });
      
      final categoryNodesJson = json.encode(categoryNodesData);
      await prefs.setString(_keyCreatedNodes, categoryNodesJson);
      
      // Save category-specific team positions
      final categoryTeamPositionsJson = json.encode(_categoryTeamPositions);
      await prefs.setString(_keyTeamPositions, categoryTeamPositionsJson);
  ...
  Future<void> _saveTournament() async {
    try {
      ...
      // Create custom bracket structure for the current category
      if (_selectedCategory != null) {
        ...
        customBrackets[_selectedCategory!] = CustomBracketStructure(
          nodes: nodes,
  ...
  void _onCategoryChanged(String? newCategory) {
    if (newCategory != _selectedCategory) {
      ...
      setState(() {
        ...
        // Load saved state for new category OR start fresh
        if (newCategory != null) {
          if (_categoryNodes.containsKey(newCategory)) {
            // Restore saved state for this category
            _createdNodes = List<Map<String, dynamic>>.from(_categoryNodes[newCategory]!);
            _teamPositions = Map.from(_categoryTeamPositions[newCategory]!);
          } else {
            ...
            final categoryTeams = _teamsByCategory[newCategory] ?? [];
            for (final team in categoryTeams) {
              _teamPositions[team.id] = 'sidebar';
            }
          }
            ...
            children: [
              ...
              SizedBox(
                ...
                child: DropdownButtonFormField<String>(
                  ...
                  items: _teamsByCategory.keys.map((category) {
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          height: 20,
                          decoration: BoxDecoration(
                            color: _getCategoryColor(_selectedCategory ?? 'Standard'),
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Text(
              'Turnier-Aufbau: $_selectedCategory',
              style: const TextStyle(
                fontSize: 20,
                fontWeight: FontWeight.bold,
                color: Colors.black87,
  ...
  Widget _buildFinalPositionsCard() {
    // Get total number of teams in the category
    final teams = _teamsByCategory[_selectedCategory] ?? [];
          ...
          if (filledTeamCount >= 2)
            Container(
              ...
              child: TextButton.icon(
                ...
                icon: Icon(
                  pool['gamesGenerated'] == true ? Icons.refresh : Icons.sports_handball,
  ...
  void _handleTeamDrop(String targetPoolId, Map<String, dynamic> dragData, int targetIndex) {
    setState(() {
      ...
      _teamPositions[droppedTeam.id] = 'pool_${_selectedCategory}_$targetPoolId';
  ...
  List<Team> _getAvailableTeams() {
    ...
    return _teamsByCategory[_selectedCategory]?.where(
                                ...
                                onAccept: (data) {
                                  if (data.containsKey('team')) {
                                    ...
                                    setState(() {
                                      ...
                                      if (origin == 'sidebar') {
                                        _teamPositions[droppedTeam.id] = 'ko_${_selectedCategory}_${node['id']}';
                                      }
                                    });
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          // Only show category selector if there are multiple categories (divisions)
          if (_teamsByCategory.length > 1) _buildCategorySelector(),
          Expanded(
            child: Row(
              children: [
